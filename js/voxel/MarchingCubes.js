/*
 * source: https://github.com/BorisTheBrave/mc-dc/blob/a165b326849d8814fb03c963ad33a9faf6cc6dea/marching_cubes_3d.py
 */

// Do marching (see "MarchingCubes.march").
function MarchingCubes(f__eval, x, y, z, adapt=false) {
  return MarchingCubes.march(f__eval, x, y, z, adapt);
}

// Evaluates given function and returns vertices as row of coordinates.
MarchingCubes.march = function (f__eval, x, y, z, adapt=false) {
  const ev = MarchingCubes.evaluate(f__eval, x, y, z);
  const faces = MarchingCubes.faces(ev);
  const coords = []; // All vertices will be saved here as row of coordinates.
  ...
  for (let i = 0; i < faces.length; ++i) {
    ...
    for (let k = 0; k < faces[i].length; ++k) {
      ...
      const t0 = 1 - (adapt ? MarchingCubes.adapt(ev[a], ev[b]) : 0.5);
      const t1 = 1 - t0;
      const va = MarchingCubes.VERTICES[a];
      const vb = MarchingCubes.VERTICES[b];
      coords.push(x + va[0] * t0 + vb[0] * t1);
      coords.push(y + va[1] * t0 + vb[1] * t1);
      coords.push(z + va[2] * t0 + vb[2] * t1);
    }
  }
  return coords;
}
...
MarchingCubes.evaluate = function (f__eval, x, y, z) {
  return Array(8).fill().map((_,i) =>
    f__eval(x + MarchingCubes.VERTICES[i][0],
            y + MarchingCubes.VERTICES[i][1],
            z + MarchingCubes.VERTICES[i][2]));
}

// Determine which case we are.
MarchingCubes.faces = function (ev) {
  // sum(2**v for v in range(8) if ev[v] > 0)
  let casei = 0;
  if (ev[0] > 0) casei |= 0x01;
  if (ev[1] > 0) casei |= 0x02;
  if (ev[2] > 0) casei |= 0x04;
  if (ev[3] > 0) casei |= 0x08;
  if (ev[4] > 0) casei |= 0x10;
  if (ev[5] > 0) casei |= 0x20;
  if (ev[6] > 0) casei |= 0x40;
  if (ev[7] > 0) casei |= 0x80;
  return MarchingCubes.CASES[casei];
}

// Returns how far you need to interpolate from a to b to get to 0.
MarchingCubes.adapt = function (a, b) {
...
MarchingCubes.tests.sphere = function(radius=2.5) {
  const coords = [];
  for (let z = 0; z < (radius + 1) * 2; ++z)
    for (let y = 0; y < (radius + 1) * 2; ++y)
      for (let x = 0; x < (radius + 1) * 2; ++x)
  {
    const f = (x, y, z) => radius - Math.hypot(x, y, z);
    coords.push(...MarchingCubes(f, x - radius, y - radius, z - radius, true));
  }
  return coords;
}

MarchingCubes.tests.voxels = function(size=7) {
  const voxels = Array(size ** 3).fill().map(Math.radnom);
  const I = (x, y, z) => {
    // Calculates map index (0 or positive, less than size ** 3), otherwise returns -1.
  ...
  {
    const f = (x, y, z) => {
      const i = I(size-x, size-y, size-z);
      return i >= 0 && (voxels[i] < 0.5);
    }
    coords.push(...MarchingCubes(f, x, y, z));
  }
  return coords;
}
