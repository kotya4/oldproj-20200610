...
function MarchingCubes() {
  ...
  const get_faces = (feval) => {
    ...
    let casei = 0;
    if (feval[0] > 0) casei |= 0x01;
    if (feval[1] > 0) casei |= 0x02;
    if (feval[2] > 0) casei |= 0x04;
    if (feval[3] > 0) casei |= 0x08;
      ...
      for (let z = 0; z < DEPTH; ++z)
  {
    ...
    for (let face of faces) {
      ...
      for (planar of planars) {
        ...
        // TODO: Здесь текущий face проверяется с текущим planar'ом на то,
        //       находится ли planar рядом с кубом, в который заключен face.
        //       Для этого исследуется объем 3х3х3 с центром planar.cube_positions
        //       (в зависимости от количества face'ов в planar'е, их там может быть несколько).
        //       Новая позиция не будет записана в planar.cube_positions, если
        //       он уже содержится в planar.cube_positions, однако, из-за свойств
        //       таблицы треугольников MarchingCubes, исследуемые позиции никогда не совпадут,
        //       т. к. куб не содержит в себе треугольники с одинаковыми нормалями.
        //       В то же время, для каждой новой позиции исследуется объем 3х3х3.
        //       Это значит, что для двух face'ов, даже если они находятся на соседних
        //       кубах, будет исследовано (3*3*3) * 2 позиции. Можно ли как-то сократить
        //       это количество? Например, запоминать вместе с самими позициями позиции окружающих
