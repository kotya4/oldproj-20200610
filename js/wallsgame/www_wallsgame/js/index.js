...
window.onload = () => {
  ...
  function define_wall(middot_speed) {
    return {
  ...
  function reset_side_wall(w2, w1) {
    w2.lx = -wx;
    w2.ly =  0;
    w2.rx =  0;
    w2.ry =  0;
  ...
  function draw_brickwall(TLx, TLy, TRx, TRy, BLx, BLy, BRx, BRy, color) {
    // L === N (left === near)
    // R === F (right === far)
  ...
  function demo(ctx) {
    ...
    const map = Array(16).fill().map((_,y)=>Array(16).fill().map((_,x)=>('翿䁁坝儑啕䐁啽䄁啷儑嗕䐑坵儅翿'.charCodeAt(y)>>x)&1));
